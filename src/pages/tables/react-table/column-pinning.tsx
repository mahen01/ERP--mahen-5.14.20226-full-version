...
function ReactTable({ columns, data }: ReactTableProps) {
  ...
  const TablePane = ({
    ...
    cols: Column<TableDataProps, unknown>[];
    colRef: RefObject<HTMLDivElement | null>;
    type: 'left' | 'center' | 'right';
    headerGroups: HeaderGroup<TableDataProps>[];
  }) => {
            ...
            <TableHead>
              {headerGroups.map((headerGroup) => (
                <TableRow key={headerGroup.id}>
                  {headerGroup.headers
                    .filter((header: any) => cols.map((c) => c.id).includes(header.column.id))
                    .map((header: any) => (
                      <TableCell
                        key={header.id}
...
export default function ColumnPinning() {
  const data: TableDataProps[] = useMemo(() => makeData(20), []);
